...
if ($exam_id <= 0 || $title === '' || $duration <= 0 || !in_array($grade, ['first', 'second', 'third']) || count($questions) === 0) {
  http_response_code(400);
...
foreach ($questions as $index => $q) {
  ...
  if (!isset($q['options']) || !is_array($q['options']) || count($q['options']) !== 4) {
    http_response_code(400);
    echo json_encode(['status' => 'error', 'message' => "السؤال $question_num لازم يكون له 4 اختيارات"], JSON_UNESCAPED_UNICODE);
    exit;
  }
  ...
  $validated_questions[] = [
    ...
    'correct_answer' => intval($q['correct_answer'])
  ];
}
...
try {
  ...
  $stmt = $pdo->prepare("
        UPDATE exams 
        SET title = :title, duration = :duration, grade = :grade, questions_json = :questions_json
        WHERE id = :exam_id
    ");
  ...
  logActivity(
    $pdo,
    'exam_updated',
    "تحديث الامتحان '{$exam['title']}' → '$title' ($questions_count سؤال، $duration دقيقة)",
    'exam',
    $exam_id
  );

  http_response_code(200);
  echo json_encode(['status' => 'success', 'message' => 'تم تحديث الامتحان بنجاح'], JSON_UNESCAPED_UNICODE);

} catch (PDOException $e) {
  error_log("[Admin Update Exam Error] " . $e->getMessage());
